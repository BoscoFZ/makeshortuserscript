...
// @downloadURL https://raw.githubusercontent.com/BoscoFZ/makeshortuserscript/release/release/ChatGPT20Infinity20E2889E.user.js
// @updateURL https://raw.githubusercontent.com/BoscoFZ/makeshortuserscript/release/release/ChatGPT20Infinity20E2889E.meta.js
// ==/UserScript==
(async()=>{const e={browser:{language:chatgpt.getUserLanguage(),isFF:chatgpt.browser.isFirefox(),isMobile:chatgpt.browser.isMobile()},scriptManager:{name:(()=>{try{return GM_info.scriptHandler}catch(e){return"unknown"}})(),version:(()=>{try{return GM_info.version}catch(e){return"unknown"}})()},ui:{scheme:r()}};e.browser.isPortrait=e.browser.isMobile&&window.innerWidth<window.innerHeight,e.scriptManager.supportsTooltips="Tampermonkey"==e.scriptManager.name&&parseInt(e.scriptManager.version.split(".")[0])>=5;const t="undefined"!=typeof GM&&GM.xmlHttpRequest||GM_xmlhttpRequest,s={version:GM_info.script.version,configKeyPrefix:"chatGPTinfinity",chatgptJSver:/chatgpt\.js@([\d.]+)/.exec(GM_info.scriptMetaStr)[1],urls:{},latestResourceCommitHash:"53e7511"};s.urls.resourceHost=`https://cdn.jsdelivr.net/gh/adamlui/chatgpt-infinity@${s.latestResourceCommitHash}`;const a=await new Promise((e=>t({method:"GET",url:`${s.urls.resourceHost}/app.json`,onload:t=>e(JSON.parse(t.responseText))})));if(Object.assign(s,{...a,urls:{...s.urls,...a.urls}}),s.urls.update=s.urls.greasyFork.replace("https://","https://update.").replace(/(\d+)-?([a-z-]*)$/i,((e,t,s)=>`${t}/${s||"script"}.meta.js`)),s.msgs={appName:s.name,appAuthor:s.author.name,appDesc:"Generate endless answers from all-knowing ChatGPT (in any language!)",menuLabel_infinityMode:"Infinity Mode",menuLabel_autoStart:"Auto-Start",menuLabel_toggleVis:"Toggle Visibility",menuLabel_autoScroll:"Auto-Scroll",menuLabel_replyLang:"Reply Language",menuLabel_replyTopic:"Reply Topic",menuLabel_all:"ALL",menuLabel_replyInt:"Reply Interval",menuLabel_about:"About",menuLabel_donate:"Please send a donation",menuLabel_disabled:"Disabled (extension installed)",about_author:"Author",about_and:"&",about_contributors:"contributors",about_version:"Version",about_poweredBy:"Powered by",about_openSourceCode:"Open source code",prompt_updateReplyLang:"Update reply language",prompt_updateReplyTopic:"Update reply topic",prompt_orEnter:"or enter",prompt_updateReplyInt:"Update reply interval (minimum 5 secs)",helptip_autoStart:"Automatically start Infinity Mode when visiting chatgpt.com",helptip_toggleVis:"Show Infinity Mode toggle in sidebar",helptip_autoScroll:"Automatically scroll to bottom as replies are generating",alert_replyLangUpdated:"Language updated",alert_willReplyIn:"will reply in",alert_yourSysLang:"your system language",alert_replyTopicUpdated:"Topic updated",alert_willAnswer:"will answer questions",alert_onTopicOf:"on the topic of",alert_onAllTopics:"on ALL topics",alert_replyIntUpdated:"Interval updated",alert_willReplyEvery:"will reply every",alert_choosePlatform:"Choose a platform",alert_updateAvail:"Update available",alert_newerVer:"An update to",alert_isAvail:"is available",alert_upToDate:"Up-to-date",alert_isUpToDate:"is up-to-date",alert_showYourSupport:"Show your support",alert_isOSS:"is open-source software built & maintained for free through 100% volunteer efforts",alert_despiteAffliction:"Despite being severely afflicted by",alert_longCOVID:"long COVID",alert_since2020:"since 2020",alert_byDonatingResults:"by donating, you help me to continue improving, fixing bugs, adding new features, and making the software even better",alert_yourContrib:"Your contribution",alert_noMatterSize:"no matter the size",alert_directlySupports:"directly supports my unpaid efforts to ensure this project remains free and open for all to use",alert_tyForSupport:"Thank you for your support",btnLabel_moreAIextensions:"More AI Extensions",btnLabel_rateUs:"Rate Us",btnLabel_getSupport:"Get Support",btnLabel_checkForUpdates:"Check for Updates",btnLabel_update:"Update",btnLabel_dismiss:"Dismiss",link_viewChanges:"View changes",unit_seconds:"seconds",state_enabled:"enabled",state_disabled:"disabled",state_on:"on",state_off:"off"},!e.browser.language.startsWith("en")){const a=await new Promise((a=>{const n=s.urls.resourceHost+"/chrome/extension/_locales/",o=(e.browser.language?e.browser.language.replace("-","_"):"en")+"/";let i=n+o+"messages.json",r=0;function l(){t({method:"GET",url:i,onload:p})}function p(t){try{const e=JSON.parse(t.responseText),s={};for(const t in e)"object"==typeof e[t]&&"message"in e[t]&&(s[t]=e[t].message);a(s)}catch(t){if(r++,3==r)return a({});i=e.browser.language.includes("-")&&1==r?i.replace(/([^_]+_[^_]+)_[^/]*(\/.*)/,"$1$2"):n+"en/messages.json",l()}}l()}));Object.assign(s.msgs,a)}dom.imports.import({env:e}),modals.imports.import({app:s,env:e,updateCheck:function(){t({method:"GET",url:s.urls.update+"?t="+Date.now(),headers:{"Cache-Control":"no-cache"},onload:e=>{if(s.latestVer=/@version +(.*)/.exec(e.responseText)?.[1],s.latestVer)for(let e=0;e<4;e++){const t=parseInt(s.version.split(".")[e],10)||0,a=parseInt(s.latestVer.split(".")[e],10)||0;if(t>a)break;if(a>t)return modals.open("update","available")}modals.open("update","unavailable")}})}}),settings.imports.import({app:s}),settings.load(Object.keys(settings.controls).filter((e=>"infinityMode"!=e))),settings.save("infinityMode",!1),config.replyLanguage||settings.save("replyLanguage",e.browser.language),config.replyTopic||settings.save("replyTopic",s.msgs.menuLabel_all),config.replyInterval||settings.save("replyInterval",7);const n={ids:[],state:{symbols:["❌","✔️"],separator:"Tampermonkey"==e.scriptManager.name?" — ":": ",words:[s.msgs.state_off.toUpperCase(),s.msgs.state_on.toUpperCase()]},register(){if(e.extensionInstalled)GM_registerMenuCommand(`${n.state.symbols[0]} ${s.msgs.menuLabel_disabled}`,(()=>modals.open("about")),e.scriptManager.supportsTooltips?{title:" "}:void 0);else{const t=new RegExp(`^(${s.msgs.menuLabel_all}|all|any|every)$`,"i");Object.keys(settings.controls).forEach((a=>{const r=config[a]^/disabled|hidden/i.test(a),l=`${settings.controls[a].symbol||n.state.symbols[+r]} `+settings.controls[a].label+("toggle"==settings.controls[a].type?n.state.separator+n.state.words[+r]:`— ${settings.controls[a].status}`);n.ids.push(GM_registerMenuCommand(l,(()=>{if("toggle"==settings.controls[a].type)settings.save(a,!config[a]),o(`${settings.controls[a].label}: ${n.state.words[+(/disabled|hidden/i.test(a)^config[a])]}`);else if("replyLanguage"==a)for(;;){let t=prompt(`${s.msgs.prompt_updateReplyLang}:`,config.replyLanguage);if(null==t)break;if(!/\d/.test(t)){t=t.length<4||t.includes("-")?t.toUpperCase():t.charAt(0).toUpperCase()+t.slice(1).toLowerCase(),settings.save("replyLanguage",t||e.browser.language),modals.alert(s.msgs.alert_replyLangUpdated+"!",s.msgs.appName+" "+s.msgs.alert_willReplyIn+" "+(t||s.msgs.alert_yourSysLang)+".");break}}else if("replyTopic"==a){let e=prompt(s.msgs.prompt_updateReplyTopic+" ("+s.msgs.prompt_orEnter+" 'ALL'):",config.replyTopic);null!=e&&(e=function(e){if(!e)return"";const t=e.toLowerCase().split(" ");for(let e=0;e<t.length;e++)t[e]=t[e][0].toUpperCase()+t[e].slice(1);return t.join(" ")}(e.toString()),settings.save("replyTopic",!e||t.test(e)?s.msgs.menuLabel_all:e),modals.alert(`${s.msgs.alert_replyTopicUpdated}!`,`${s.msgs.appName} ${s.msgs.alert_willAnswer} `+(!e||t.test(e)?s.msgs.alert_onAllTopics:`${s.msgs.alert_onTopicOf} ${e}`)+"!"))}else if("replyInterval"==a)for(;;){const e=prompt(`${s.msgs.prompt_updateReplyInt}:`,config.replyInterval);if(null==e)break;if(!isNaN(parseInt(e,10))&&parseInt(e,10)>4){settings.save("replyInterval",parseInt(e,10)),modals.alert(s.msgs.alert_replyIntUpdated+"!",s.msgs.appName+" "+s.msgs.alert_willReplyEvery+" "+e+" "+s.msgs.unit_seconds+".");break}}i({updatedKey:a})}),e.scriptManager.supportsTooltips?{title:settings.controls[a].helptip||" "}:void 0))}))}["about","donate"].forEach((t=>{"donate"===t&&e.extensionInstalled||n.ids.push(GM_registerMenuCommand(("about"==t?"💡":"💖")+` ${s.msgs[`menuLabel_${t}`]} ${"about"==t?s.msgs.appName:""}`,(()=>modals.open(t)),e.scriptManager.supportsTooltips?{title:" "}:void 0))}))},refresh(){if("undefined"!=typeof GM_unregisterMenuCommand){for(const e of n.ids)GM_unregisterMenuCommand(e);n.register()}}};function o(t,a="",o="",i=""){const r=n.state.words.find((e=>t.includes(e)));r&&(t=t.replace(r,"")),chatgpt.notify(`${s.symbol} ${t}`,a,o,i||"dark"==e.ui.scheme?"":"shadow");const l=document.querySelector(".chatgpt-notif:last-child");if(r){const e=dom.create.elem("span");e.style.cssText="color: "+(r==n.state.words[0]?"#ef4848 ; text-shadow: rgba(255,169,225,0.44) 2px 1px 5px":"#5cef48 ; text-shadow: rgba(255,250,169,0.38) 2px 1px 5px"),e.append(r),l.append(e)}}function i(e){"infinityMode"==e?.updatedKey?l[config.infinityMode?"activate":"deactivate"]():"prompt"==settings.controls[e?.updatedKey].type&&config.infinityMode&&l.restart({target:"replyInterval"==e?.updatedKey?"self":"new"}),/infinityMode|toggleHidden/.test(e?.updatedKey)&&toggles.sidebar.updateState(),n.refresh()}function r(){return document.documentElement.className||(window.matchMedia?.("(prefers-color-scheme: dark)")?.matches?"dark":"light")}chatgpt.isIdle=function(){return new Promise((e=>{new MutationObserver(((t,s)=>{chatgpt.getStopBtn()||(s.disconnect(),e())})).observe(document.body,{childList:!0,subtree:!0})}))};const l={async activate(){const t="Generate a single random question"+(config.replyLanguage?" in "+config.replyLanguage:"")+" on "+("ALL"==config.replyTopic?"ALL topics":"the topic of "+config.replyTopic)+" then answer it. Don't type anything else.";if(e.browser.isMobile&&chatgpt.sidebar.isOn()&&chatgpt.sidebar.hide(),!new URL(location).pathname.startsWith("/g/"))try{chatgpt.startNewChat()}catch(e){return}await new Promise((e=>setTimeout(e,500))),chatgpt.send(t),await new Promise((e=>setTimeout(e,3e3))),!document.querySelector("[data-message-author-role]")&&config.infinityMode&&chatgpt.send(t),await chatgpt.isIdle(),config.infinityMode&&!l.isActive&&(l.isActive=setTimeout(l.continue,1e3*parseInt(config.replyInterval,10)))},async continue(){if(!config.autoScrollDisabled)try{chatgpt.scrollToBottom()}catch(e){}chatgpt.send("Do it again."),await chatgpt.isIdle(),l.isActive&&(l.isActive=setTimeout(l.continue,1e3*parseInt(config.replyInterval,10)))},deactivate(){chatgpt.getStopBtn()&&chatgpt.stop(),clearTimeout(l.isActive),l.isActive=null},async restart(e={target:"new"}){"new"==e.target?(l.deactivate(),setTimeout((()=>l.activate()),750)):(clearTimeout(l.isActive),l.isActive=null,await chatgpt.isIdle(),config.infinityMode&&!l.isActive&&(l.isActive=setTimeout(l.continue,1e3*parseInt(config.replyInterval,10))))}};function p(){const t=r();e.ui.scheme!=t&&(e.ui.scheme=t,toggles.sidebar.updateAesthetic(),modals.stylize())}e.extensionInstalled=await Promise.race([new Promise((e=>{!function t(){document.documentElement.hasAttribute("chatgpt-infinity-extension-installed")?e(!0):setTimeout(t,200)}()})),new Promise((e=>setTimeout((()=>e(!1)),1500)))]),n.register(),e.extensionInstalled||(await Promise.race([chatgpt.isLoaded(),new Promise((e=>setTimeout(e,5e3)))]),await chatgpt.sidebar.isLoaded(),e.ui.firstLink=chatgpt.getNewChatLink(),"hidden"in document&&(document.onvisibilitychange=()=>{config.infinityMode&&(settings.save("infinityMode",!1),i({updatedKey:"infinityMode"}))}),["brs","wrs"].forEach((e=>document.head.append(dom.create.style(GM_getResourceText(`${e}CSS`))))),toggles.imports.import({app:s,env:e,notify:o,syncConfigToUI:i}),toggles.sidebar.insert(),config.autoStart&&(settings.save("infinityMode",!0),i({updatedKey:"infinityMode"}),o(`${s.msgs.menuLabel_autoStart}: ${s.msgs.state_on.toUpperCase()}`)),new MutationObserver((()=>{config.toggleHidden||document.querySelector(`.${toggles.sidebar.class}`)||"inserting"==toggles.sidebar.status||(toggles.sidebar.status="missing",toggles.sidebar.insert())})).observe(document.body,{attributes:!0,subtree:!0}),new MutationObserver(p).observe(document.documentElement,{attributes:!0,attributeFilter:["class"]}),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",(()=>requestAnimationFrame(p))),document.documentElement.hasAttribute("sidebar-click-zoom-observed")||(new MutationObserver((e=>e.forEach((({target:e})=>{e.closest("[class*=sidebar]")&&!e.closest("[class*=sidebar-toggle]")&&"none"!=e.style.transform&&(e.style.transform="none")})))).observe(document.body,{attributes:!0,subtree:!0,attributeFilter:["style"]}),document.documentElement.setAttribute("sidebar-click-zoom-observed",!0)))})();